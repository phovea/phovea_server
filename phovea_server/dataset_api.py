...
from .plugin import list
from .ns import Namespace, request, etag, Response, abort, make_response
...
def _list_format_csv(data):
    ...
    return Response(gen(), mimetype='text/csv', headers={'Content-Disposition': 'attachment;filename=dataset.csv'})
...
def _list_datasets():
    if request.method == 'GET':
        ...
        if format not in formats:
            abort(
                make_response('invalid format: "{0}" possible ones: {1}'.format(format, ','.join(list(formats.keys()))), 400))
...
for handler in list('dataset-specific-handler'):
    p = handler.load()
