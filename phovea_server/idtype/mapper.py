...
# Define type of idtype mapping tuple.
IDTypeMappingFunc = Callable[[List[str]], List[List[str]]]
IDTypeMapping = Tuple[str, str, IDTypeMappingFunc]


class MappingManager(object):
    """
    Constructs a graph of idtype mappings and enables the (transitive) translation between them.
    """

    def __init__(self, providers: List[IDTypeMapping]):
        self.mappers: Dict[str, Dict[str, IDTypeMappingFunc]] = {}
        self.paths: Dict[str, Dict[str, List[List[str]]]] = {}
        self.providers: List[IDTypeMapping] = providers

        self.initialize_graph()

    def initialize_graph(self):
        _log.info(f'Initializing graph with {len(self.providers)} providers')
        graph = {}
        for (from_idtype, to_idtype, mapper) in self.providers:
    ...
    def add_mappings(self, mappings: List[IDTypeMapping]):
        self.providers.extend(mappings)
        self.initialize_graph()

    def known_idtypes(self) -> Set[str]:
        """
    ...
    def __find_all_paths(self, graph, start, end, path=[]) -> List[List[str]]:
        """
...
def create():
    ...
    for plugin in list_plugin('mapping_provider'):
        new_mappings = list(plugin.load().factory() or [])
        _log.info(f'Adding {len(new_mappings)} idtype mappings for mapping provider {plugin.id} ({plugin.module})')
        providers = providers + new_mappings
